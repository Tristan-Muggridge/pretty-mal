...
func main() {
	...
	r.GET("/manga/top", func(c *gin.Context) {
		...
		url := "https://api.myanimelist.net/v2/manga/ranking?limit=100&fields=id,title,main_picture,alternative_titles,start_date,end_date,synopsis,mean,rank,popularity,num_list_users,num_scoring_users,nsfw,created_at,updated_at,media_type,status,genres,my_list,num_volumes,num_chapters,authors"
		resp := request(MALclient, url)
		defer resp.Body.Close()
